...
- `session_meta` establishes session ID, working directory, and model
...
- Legacy sessions are rewritten full JSON documents with a `messages` array
- Current sessions are append-style JSONL records whose `$set.messages` arrays can exceed one megabyte
