...
public actor IndexCoordinator {
    ...
    private struct EffectiveCodexNames {
        ...
        let partialLocalNames: [String: String]
        let warnings: [String]
    }
    ...
    private func effectiveCodexNames(
        ...
        guard let defaultRoot = source.roots.first(where: \.isDefault) ?? source.roots.first
            ...
            return .init(
                ...
                fillMissingOnly: false, partialLocalNames: [:], warnings: []
            )
        }
        ...
        var completeLocalProvider = false
        var partialLocalNames: [String: String] = [:]
        ...
        for directory in directories {
            ...
            case .loaded(let loaded, let providerComplete, let databaseFailed, let warning):
                foundProvider = true
                if isLocal {
                    completeLocalProvider = providerComplete
                    if !providerComplete && !databaseFailed {
                        partialLocalNames = loaded
                    }
                }
        ...
        if inheritsDefault && localPath != defaultPath && !completeLocalProvider {
            complete = false
            fillMissingOnly = true
        }
        return .init(
            names: names, hasProvider: foundProvider, complete: complete,
            fillMissingOnly: fillMissingOnly, partialLocalNames: partialLocalNames,
                ...
                for root in source.roots {
                    do {
                        ...
                        if metadata.hasProvider {
                            ...
                            if metadata.fillMissingOnly && !metadata.partialLocalNames.isEmpty,
                               try await database.updateCodexNames(
                                metadata.partialLocalNames, root: root.url,
                                policy: .replaceMatches
                               ) {
                                await mutations.markChanged()
                            }
                        }
