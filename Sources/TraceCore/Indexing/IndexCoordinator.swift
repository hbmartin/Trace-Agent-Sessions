...
extension IndexActivity {
    ...
    public static func moreSignificant(
        ...
        func rank(_ activity: IndexActivity) -> Int {
            ...
            case .eventStreamRecovery: 5
            case .safetyVerification: 6
            case .initialBuild: 7
            case .rootRecovery: 8
            case .scopeChange: 9
            case .rebuild: 10
...
public actor IndexCoordinator {
    ...
    private struct RootDiscoveryAttempt {
        ...
        let root: SourceRoot
        let mappingContext: SourceRootPathContext
    ...
    private enum CodexWarningKind: Hashable {
        case metadata
        case metadataLoad
        case lookup
        case fileWrite
    ...
    private struct CodexWarningKey: Hashable {
        let rootID: String
        let kind: CodexWarningKind
    ...
    private func replaceCodexWarnings(
        _ warnings: [String], kind: CodexWarningKind, for rootID: String,
        sourcePath: String? = nil, in status: inout IndexProgress
    ) {
        let key = CodexWarningKey(rootID: rootID, kind: kind, sourcePath: sourcePath)
        if warnings.isEmpty { codexWarnings.removeValue(forKey: key) }
        else { codexWarnings[key] = Set(warnings) }
        status.metadataWarning = codexWarningMessage()
    }

    private func clearFileCodexWarnings(for rootID: String, in status: inout IndexProgress) {
        codexWarnings = codexWarnings.filter { key, _ in
            key.rootID != rootID || (key.kind != .lookup && key.kind != .fileWrite)
        }
        status.metadataWarning = codexWarningMessage()
    }
    ...
    private func codexNames(
        directory: URL, retryIncomplete: Bool = false
    ...
    private func codexMetadataDirectories(
        ...
        let caseSensitive = defaultRoot.scanPath.isCaseSensitive
        let configuredRoot = TraceFileIO.comparisonKey(
            root.url.path, caseSensitive: caseSensitive
        )
        let configuredDefault = TraceFileIO.comparisonKey(
            defaultRoot.url.path, caseSensitive: caseSensitive
        )
        let configuredNested = configuredRoot == configuredDefault
            || configuredRoot.hasPrefix(configuredDefault + "/")
        ...
        if (!aliasIntoDefault || hasCodexMetadataFile(in: localDirectory))
    ...
    private func effectiveCodexNames(
        for root: SourceRoot, in source: any SessionSource,
        retryIncomplete: Bool = false
    ) async throws -> EffectiveCodexNames {
        guard let defaultRoot = source.roots.first(where: \.isDefault) ?? source.roots.first
        else {
            return .init(
        ...
        for directory in directories {
            ...
            let isLocal = directory.standardizedFileURL.path
    ...
    public func indexAll(scope: IndexScope, rebuild: Bool = false,
        ...
        _ = await indexAllResult(
            scope: scope, rebuild: rebuild, activity: activity,
    ...
    public func indexAllResult(scope: IndexScope, rebuild: Bool = false,
        ...
        await gate.acquire()
        let activity = activity ?? (rebuild ? .rebuild : .initialBuild)
        let result = await run(
            scope: scope, paths: nil, reconciliationPaths: [], rebuild: rebuild,
            activity: activity,
            satisfiesSafetyReconciliation: satisfiesSafetyReconciliation
                ?? activity.satisfiesSafetyReconciliation,
            progress: progress
        )
        await gate.release()
        return result
    ...
    public func refresh(paths: Set<String>, scope: IndexScope,
        ...
        _ = await refreshResult(
            paths: paths, scope: scope, activity: activity,
            satisfiesSafetyReconciliation: satisfiesSafetyReconciliation,
            progress: progress
        )
    }
    ...
    public func reconcile(paths: Set<String>, changedPaths: Set<String> = [], scope: IndexScope,
        ...
        let result = await run(
            ...
            rebuild: false, activity: activity,
            satisfiesSafetyReconciliation: satisfiesSafetyReconciliation
                ?? activity.satisfiesSafetyReconciliation,
            progress: progress
        )
    ...
    private func run(scope: IndexScope, paths: Set<String>?, reconciliationPaths: Set<String>,
                     ...
                     satisfiesSafetyReconciliation: Bool,
        ...
        do {
            ...
            for source in sources where source.agent == .codex {
                for root in source.roots {
                    let directories = codexMetadataDirectories(for: root, in: source)
                    // Match the configured home even after its last sidecar disappears.
                    // The loader still excludes an unrelated alias parent with no sidecar.
                    let changeCandidates = directories + [Self.codexMetadataDirectory(for: root)]
                    let changed = changeCandidates.filter {
                    ...
                    invalidatedMetadataDirectories.formUnion(
                        changeCandidates.map { $0.standardizedFileURL.path }
                    )
                }
            }
            ...
            func recordDiscoveryFailure(
                _ error: Error, source: any SessionSource, root: SourceRoot,
                mappingContext: SourceRootPathContext, scopes: [SourceRootScope]
            ) {
                ...
                let failures = scopes.map { scope in
                    NormalizedDiscoveryFailure(
                        failure: DiscoveryFailure(
                            ...
                            path: scope.scanPath.path, message: message
                        ),
                        scope: scope
                    )
                }
                    ...
                    for root in source.roots {
                        ...
                        do {
                            ...
                            recordDiscoveryFailure(
                                error, source: source, root: root,
                                mappingContext: context, scopes: [scope]
                            )
                        }
                    }
                }
                        ...
                        for root in source.roots {
                            ...
                            do {
                                ...
                                absorbDiscoveryResult(
                                    ...
                                    mappingContext: context, scopes: scopes
                                )
                            } catch is CancellationError {
                                throw CancellationError()
                            } catch {
            ...
            for file in allFiles {
                ...
                let existingID = "\(existing.agent.rawValue):\(existing.root.path)"
                let candidateID = "\(file.agent.rawValue):\(file.root.path)"
                guard let existingRoot = rootsByID[existingID],
                      let candidateRoot = rootsByID[candidateID] else { continue }
                if Self.prefers(candidateRoot, over: existingRoot) {
                    uniqueFiles[key] = file
                }
            }
            ...
            for file in allFiles {
                ...
                if shouldRefreshMissingCodexTitle {
                    var warningKind = CodexWarningKind.lookup
                    do {
                        if let state = try await database.sourceState(
                           ...
                           try await database.hasUntitledCodexSessions(sourceID: state.id) {
                            ...
                            if let root = rootsByID[rootKey] {
                                warningKind = .metadataLoad
                                let metadata = try await effectiveCodexNames(
                                    for: root, in: source
                                )
                                replaceCodexWarnings([], kind: .metadataLoad,
                                                     for: root.id, in: &status)
                                replaceCodexWarnings(metadata.warnings, kind: .metadata,
                                                     for: root.id, in: &status)
                                if !metadata.names.isEmpty {
                                    warningKind = .fileWrite
                                    ...
                                    replaceCodexWarnings([], kind: .fileWrite,
                                                         for: root.id, sourcePath: file.url.path,
                                                         in: &status)
                                }
                            }
                        } else {
                        ...
                        replaceCodexWarnings(
                            [error.localizedDescription], kind: warningKind,
                            for: rootKey,
                            sourcePath: warningKind == .lookup || warningKind == .fileWrite
                                ? file.url.path : nil,
                            in: &status
                        )
                    }
            ...
            if !missingSources.isEmpty {
                ...
                for missing in missingSources.sorted(by: {
                    ...
                    if let state = try await database.sourceState(
                        ...
                        try await database.deleteSource(id: state.id)
                        await mutations.markChanged()
                    }
                }
                updateFailureCounts(&status, try? await database.unresolvedSourceFailureCounts())
            ...
            if fullScan || !reconciliationPaths.isEmpty {
                ...
                for source in sources {
                    ...
                    where !live.contains(SourceFileKey(
                        ...
                        let relative = attempt.root.relativeScope(
                            forStoredPath: stored.path,
                            in: attempt.mappingContext
                        )
                        if coversWholeRoot {
                            if failedScopes.contains(where: \.isRoot) {
                                continue
                            }
            ...
            for source in sources where source.agent == .codex && refreshCodexNames {
                for root in source.roots where codexRefreshRoots.contains(root.id) {
                    var warningKind = CodexWarningKind.metadataLoad
                    do {
                        let metadata = try await effectiveCodexNames(
                        ...
                        guard !rootIsMissing, rootIsAvailableForCodexNames(root),
                              let rootID = rootIDs[root.id],
                              !(try await database.rootHasUnresolvedRootFailure(rootID: rootID))
                        else { continue }
                        warningKind = .rootWrite
                        ...
                        if update.changed { await mutations.markChanged() }
    ...
    private static func minimalScopes(_ paths: Set<String>) -> Set<String> {
        Set(minimalCanonicalScopes(paths.map(TraceFileIO.canonicalPath)).map(\.path))
    }

    private static func minimalCanonicalScopes(
        _ paths: [TraceFileIO.CanonicalPath]
    ) -> [TraceFileIO.CanonicalPath] {
        var unique: [String: TraceFileIO.CanonicalPath] = [:]
        for path in paths {
            if let existing = unique[path.comparisonKey], existing.path <= path.path { continue }
            unique[path.comparisonKey] = path
        }
        let canonical = Array(unique.values)
    ...
    private nonisolated static func loadCodexNames(
        ...
        let task = Task.detached(priority: .utility) {
            try CodexSessionNames.load(directory: directory)
        }
        return try await withTaskCancellationHandler(
            operation: { try await task.value },
            onCancel: { task.cancel() }
    ...
    private func process(
        ...
        if let existing = state, existing.format != file.format {
    ...
    private func classification(
        ...
        var contexts: [String: SourceRootPathContext] = [:]
        for source in sources where source.agent == agent {
            for root in source.roots where contexts[root.id] == nil {
                contexts[root.id] = root.mappingContext
            }
        }
            ...
            for root in source.roots {
                ...
                if let format {
                    ...
                    if let existing = matches[source.agent] {
                        if !Self.prefers(root, over: existing.2) { continue }
                    }
    ...
    private static func prefers(_ candidate: SourceRoot, over existing: SourceRoot) -> Bool {
        candidate.rootSelectionDepth > existing.rootSelectionDepth
            || (candidate.rootSelectionDepth == existing.rootSelectionDepth
